...
def get_json(to_match):
    ...
    try:
        ...
        print(f"Invalid line found: {match.group()}")
...
def prepare_to_plot(sheet_name, __data):
    ...
    try:
        if sheet_name == "HST2":
            ...
            return to_dataframe
        elif sheet_name == "DST2":
            to_dataframe = get_dataframe(__data).drop(columns=["CID", "VALUES"])
            return to_dataframe
        elif sheet_name == "DSTU":
            to_dataframe = get_dataframe(__data).drop(columns=["MODE", "PANGLE", "CANGLE", "BTEMP", "BATV"])
            return to_dataframe
    except:
        pass
...
def filter_dataframe_by_timeStamp(dataframe1, dataframe2):
    """
    ...
    count_df["Commands missed count"] = count_df['HST2 Command Count'] - count_df['DST2 Command Count']
    return count_df


def prepare_plot(plot_name, _dataframe):
    _dataframe["TS"] = _dataframe['TS'].dt.date
    pivot_table = _dataframe.pivot_table(values=["HST2 Command Count", "DST2 Command Count"], index='TS', aggfunc=sum)
    ax = pivot_table.plot(kind='bar')
    ax.set_yticks([])
    ax.set_yticklabels([])
    plt.xlabel("Date")
    plt.ylabel("HST2 Sent and DST2 Received Count")
...
if __name__ == "__main__":
    for i, _ in enumerate(LOGSDIRs):
        ...
        excelWriter(excel_file, ["HST2", "DST2", "DSTU"], [HST2, DST2, DSTU])
        hst2 = prepare_to_plot("HST2", get_dataframe(HST2))
        dst2 = prepare_to_plot("DST2", get_dataframe(DST2))
        dstu = prepare_to_plot("DSTU", get_dataframe(DSTU))
        final_Df = filter_dataframe_by_timeStamp(hst2, dst2)
        excelWriter(excel_file_plots, ["HST2 vs DST2", "DSTU"], [final_Df, dstu])
