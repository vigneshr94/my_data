...
def log_reader(url, port, host_name, host_password):
    ...
    for x in files:
        print(f"Downloading {x} log...")
        scp.get(f"{logs_path}/{x}", "./logs")
    print("Reading log files for errors...")
    for file in files:
...
# Bluetooth
def check_bluetooth(url, port, host_name, host_password):
    print("Checking Bluetooth....")
    ssh.connect(url, port, host_name, host_password)
    stdin, stdout, stderr = ssh.exec_command("bluetoothctl power on && bluetoothctl show")
    time.sleep(20)
    cmd_output = [x.strip() for x in stdout.readlines()]
    if "Powered: yes" in cmd_output:
...
def log_data_write_excel(final_file):
    ...
    df2["log_file"] = log_f
    df2["Error"] = error_msg
    df2.to_excel(final_file, sheet_name="log_errors", index=False)
...
def system_metrics_excel_writer(url, port, host_name, host_password, final_file):
    ...
    df3.to_excel(final_file, sheet_name="System Data", index=False)
...
def service_check_excel_writer(url, port, host_name, host_password, final_file):
    ...
    df_act.to_excel(final_file, sheet_name="Active Services", index=False)
    df_inact.to_excel(final_file, sheet_name="Inactive Services", index=False)

def main(url):
    try:
        cleaning(url, port, host_name, host_password)
    except:
        print("No files to clean\ncontinue to download the files...")
    os.makedirs("./logs")
    print("Downloading logs and reading for errors...")
    log_reader(url, port, host_name, host_password)
    print("Generating excel file...")
    with pd.ExcelWriter(f"./log_errors_system_data_{str(datetime.datetime.now()).replace(':','_').split('.')[0]}.xlsx") as final_file:
